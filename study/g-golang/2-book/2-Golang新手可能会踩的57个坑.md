...
抄写自翻译的开源书籍《Golang新手可能会踩的50个坑》，其中感觉写的不是很通俗易懂，或者不是很详尽的地方进行了代码解释、描述和代码补充。
...
> map 中的每个键被视为一个独立的实体，Go 语言使用哈希表（hash table）实现 map 的数据结构，在哈希表中，每个键都会被计算出一个哈希码，用于快速查找该键所对应的值。如果您尝试访问一个不存在于 map 中的键，则哈希表会根据该键的哈希码尝试查找其对应的位置，但通常情况下，这个位置上并不存储任何数据，因此会返回该值类型的零值。
...
程序**默认不等所有 goroutine 都执行完才退出**，这点需要特别注意：
...
现在的程序中 ch <- "cmd.2" 不会被接收处理是因为当前的协程（goroutine）在运行时，只能在 for m := range ch 的语句中等待通道的下一个值。如果主协程向通道 ch 发送了第二个消息，但由于在 for 循环中等待读取通道的操作，第二个消息将得不到处理

为了让 cmd.2 能被接收并处理，可以在主协程中，先将两个消息都发送到通道中，然后再启动一个新的协程从通道中读取消息。
...
先定义了一个通道 ch，然后启动了一个协程来读取通道中的消息。在主协程中，我们依次向通道 ch 发送了两个消息，然后通过 time.Sleep(3 * time.Second) 的方式，等待协程中的所有消息都被处理完毕
...
```go
// 定义了一个包含 3 个发送者（sender）和一个接收者（receiver）的程序。具体地，我们依次启动了 3 个协程，让它们向通道 ch 中发送数字 0、1 和 2。
//然后，在主协程中，我们通过 <-ch 的方式读取通道 ch 中的第一个值，并将其输出到控制台上。
func main() {
    ch := make(chan int)
    for i := 0; i < 3; i++ {
        go func(idx int) {
